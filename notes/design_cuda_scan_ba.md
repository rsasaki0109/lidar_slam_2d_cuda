...
- **分離ベンチ（solve のみ, m=16 RHS, GPU, `tools/bench_joint_gpu_solver.py` 系）**: splu は V に対し線形〜超線形（V=1k:11ms → 5k:62ms → 10k:131ms → 40k:539ms）、**PCG はほぼ一定 ~8–11ms**（対角優勢で反復数が V に依らない）。speedup V=1k:1.39x → 5k:7.4x → 10k:16x → **40k:49.9x**。P3.5 で「splu が ~10ms/iter で壁」と言った数値は PCG では消える。
...
### P-loop 実バッグ定量 (2026-05-26): ループ閉じ込みは実データで明確な勝ち、ロバスト化は「整地された実データ」では緩ゲートでも引き分け（正直な結果）
